...
class Customer():
    ...
    @classmethod
    def add_customer(cls,data):
        query = "INSERT INTO customers (first_name,last_name,email,password) VALUES (%(first_name)s,%(last_name)s,%(email)s,%(hashed_pw)s);"
    ...
    def user_update_validator(cls,data):
        ...
        if not EMAIL_REGEX.match(data['email']):
            flash("Email is invalid. Please try again.", category='error')
            is_valid = False
        query2 = "SELECT * FROM customers WHERE id = %(id)s;"
        results2 = connectToMySQL('coffee_shop').query_db(query2,data)
        user2 = cls(results2[0])
        print(user2)
        if data['email'] != session['customer_email']:
            query = "SELECT * FROM customers WHERE email = %(email)s;"
            results = connectToMySQL('coffee_shop').query_db(query,data)
            if len(results) != 0:
                flash("Email already exist.Try logging in.", category='error')
